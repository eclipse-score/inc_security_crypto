...
namespace
{
constexpr std::string_view LOG_PREFIX = "[SCORE_CERT_MGMT_HANDLER] ";
}

ScoreCertManagementHandler::ScoreCertManagementHandler(
    std::unique_ptr<crypto_executor::CertManagementExecutor> executor)
    : m_executor{std::move(executor)}, m_ctx{}
{
}

Expected<std::monostate, common::DaemonErrorCode> ScoreCertManagementHandler::InitializeContext(
...
Expected<common::ResponseParameters, common::DaemonErrorCode> ScoreCertManagementHandler::Execute(
...
Expected<std::monostate, common::DaemonErrorCode> ScoreCertManagementHandler::Reset()
{
    return std::monostate{};
}
