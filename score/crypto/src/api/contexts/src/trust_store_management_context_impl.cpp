    ...
    score::cpp::span<const uint8_t> sha256_fingerprint)
{
    ...
    auto req = proto::ControlRequestBuilder()
                   ...
                   .operation(op_id)
                   .with_in_val_uint64(trust_store.id)
                   .with_in_data_buffer(sha256_fingerprint)
                   .build();
    if (!req.has_value())
        return score::Result<std::monostate>{
            score::unexpect, MakeError(CryptoErrorCode::kOperationFailed, "Failed to build TRUST_STORE_REMOVE_CERT")};
    auto resp = m_connection->SendRequest(req.value());
    auto validator = proto::ControlResponseValidator::FromResult(resp);
    validator.expectOperation(op_id).expectSuccess();
    if (!validator.isValid())
    ...
    FormatType format)
{
    ...
    if (!req.has_value())
        return score::Result<std::monostate>{
            score::unexpect,
            MakeError(CryptoErrorCode::kOperationFailed, "Failed to build TRUST_STORE_IMPORT_CRL_FOR_MEMBER")};
    auto resp = m_connection->SendRequest(req.value());
    auto validator = proto::ControlResponseValidator::FromResult(resp);
    validator.expectOperation(op_id).expectSuccess();
    if (!validator.isValid())
