...
unsafe fn generate_chacha20_key(h: CK_SESSION_HANDLE) -> CK_OBJECT_HANDLE {
    let fl = common::fn_list();
    let mech = CK_MECHANISM {
        mechanism: CKM_CHACHA20_KEY_GEN,
        pParameter: ptr::null(),
        ulParameterLen: 0,
    };
...
fn test_get_interface_default() {
    ...
    unsafe {
        ...
        let rv = p11!(fl3, C_GetInterface, ptr::null(), ptr::null_mut(), &mut iface_ptr, 0,);
        assert_eq!(rv, CKR_OK);
...
fn test_mechanism_list_v3() {
    ...
    unsafe {
        ...
        assert!(
            list.contains(&CKM_EC_EDWARDS_KEY_PAIR_GEN),
            "missing CKM_EC_EDWARDS_KEY_PAIR_GEN"
        );
...
fn test_init_token_and_pin_management() {
    ...
    unsafe {
        ...
        let h = open_session();
        assert_eq!(
            p11!(fl, C_Login, h, CKU_SO, so_pin.as_ptr(), so_pin.len() as CK_ULONG),
            CKR_OK
        );
        ...
        assert_eq!(
            p11!(fl, C_InitPIN, h, new_user_pin.as_ptr(), new_user_pin.len() as CK_ULONG),
            CKR_OK
        );
