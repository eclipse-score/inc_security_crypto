...
unsafe fn do_wrap(session: CK_SESSION_HANDLE, wrapping_key: CK_OBJECT_HANDLE, target_key: CK_OBJECT_HANDLE) -> CK_RV {
    let fl = common::fn_list();
...
unsafe fn get_key_gen_mechanism(session: CK_SESSION_HANDLE, handle: CK_OBJECT_HANDLE) -> CK_ULONG {
    let fl = common::fn_list();
    ...
    assert_eq!(
        rv, CKR_OK,
        "C_GetAttributeValue(CKA_KEY_GEN_MECHANISM) failed: {rv:#010x}"
    );
    val
}
...
fn wrap_key_without_wrap_flag_rejected() {
    ...
    unsafe {
        ...
        let wrap_key = make_aes_key(session, &[(CKA_EXTRACTABLE, vec![CK_TRUE])]);
        // Target key: extractable.
        let target = make_aes_key(session, &[(CKA_EXTRACTABLE, vec![CK_TRUE])]);
...
fn wrap_non_extractable_target_rejected() {
    ...
    unsafe {
        ...
        let wrap_key = make_aes_key(session, &[(CKA_EXTRACTABLE, vec![CK_TRUE]), (CKA_WRAP, vec![CK_TRUE])]);
...
fn wrap_with_trusted_requires_trusted_key() {
    ...
    unsafe {
        ...
        let target = make_aes_key(
            session,
            &[(CKA_EXTRACTABLE, vec![CK_TRUE]), (CKA_WRAP_WITH_TRUSTED, vec![CK_TRUE])],
        );
...
fn unwrapped_key_is_not_local() {
    ...
    unsafe {
        ...
        let unwrap_template: &[(CK_ATTRIBUTE_TYPE, Vec<u8>)] =
            &[(CKA_TOKEN, vec![CK_FALSE]), (CKA_EXTRACTABLE, vec![CK_TRUE])];
