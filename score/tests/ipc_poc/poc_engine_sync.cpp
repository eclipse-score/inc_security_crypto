...
//
// This POC mirrors poc_engine, but uses a bootstrap connection to create one
// on-demand message_passing endpoint/Engine session per client thread. Each
// child connection uses blocking IClientConnection::SendWaitReply().
// =============================================================================

/// Usage:
///   bazel run //tests/score_com_poc:poc_engine_sync
///   bazel run //tests/score_com_poc:poc_engine_sync -- --call_count=5
///   bazel run //tests/score_com_poc:poc_engine_sync -- --client_threads=3 --call_count=5
///   bazel run //tests/score_com_poc:poc_engine_sync -- --call_count=3 --sleep_milliseconds=10
...
static constexpr std::string_view kBootstrapServiceIdentifier{"score_crypto_poc_engine_sync_bootstrap"};
...
static bool StartEngineSession(const std::shared_ptr<EngineSession>& session)
{
    ...
    const auto request_callback = [](score::message_passing::IServerConnection& connection,
        ...
        const auto response_bytes = helper::BuildResponseBytes(workload_response);
        ...
        Log("[server/engine] processed request_id=" + std::to_string(workload_response.request_id) +
...
static bool RunServer(const pid_t client_pid, const int thread_count)
{
    ...
    tests::utility::PrintProcessResourceDelta("poc_engine_sync_server_thread_creation",
    ...
    tests::utility::PrintProcessResourceDelta(
        "poc_engine_sync_server_workload", resources_after_thread_creation, resources_after_workload);
...
static bool RunBootstrapHandshake(const int thread_index, std::string& endpoint_identifier)
{
    ...
    std::vector<std::uint8_t> response_buffer(sizeof(IpcBuffer));
    const auto response = client->SendWaitReply(
    ...
    if (!helper::ParseResponseBytes(response->data(), response->size(), parsed_response) ||
        parsed_response.request_id != request.request_id || parsed_response.string_value.empty())
    {
        LogErr("[client " + std::to_string(thread_index) + "] invalid bootstrap response");
        client->Stop();
        return false;
    }
    endpoint_identifier = std::move(parsed_response.string_value);
                            ...
                            helper::ThreadStartBarrier& connection_ready_barrier)
{
    std::string endpoint_identifier;
    {
        score::crypto::daemon::common::RuntimeMeasurement::ScopedMeasurement setup_scope{
            latency_measurement, "POC::EngineSync::ClientThreadCreationAndBootstrapHandshake"};
    ...
    {
        score::crypto::daemon::common::RuntimeMeasurement::ScopedMeasurement setup_scope{
            latency_measurement, "POC::EngineSync::ClientConnectionSetup"};
    ...
    {
        {
            ...
            Log("[client] SendWaitReply request_id=" + std::to_string(request_id));
            const auto response = client->SendWaitReply(
                score::cpp::span<const std::uint8_t>{request_bytes.data(), request_bytes.size()},
                score::cpp::span<std::uint8_t>{reply_buffer.data(), reply_buffer.size()});
            if (!response.has_value())
            {
                LogErr("[client " + std::to_string(thread_index) + "] SendWaitReply failed for request_id=" +
            ...
            {
                LogErr("[client " + std::to_string(thread_index) + "] invalid or unexpected response for request_id=" +
                       std::to_string(request_id));
                success = false;
                break;
            }
...
static bool RunClient(const int thread_count)
{
    ...
    tests::utility::PrintProcessResourceDelta("poc_engine_sync_client_connection_thread_creation",
                                              resources_before_thread_creation,
    ...
    const auto resources_after_workload = tests::utility::CaptureProcessResourceSnapshot();
    tests::utility::PrintProcessResourceDelta(
        "poc_engine_sync_client_workload", resources_after_connection_thread_creation, resources_after_workload);
