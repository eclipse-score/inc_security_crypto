...
/// @file score_api_key_permissions_test.cpp
...
///
/// Covered:
///   - MAC needs kMac, on every provider the build enables
...
/// The keyless cases need a provider but do not depend on which: no key means no
/// grant to check, whichever provider serves the context.
#ifdef SCORE_CRYPTO_SOFTWARE_BACKEND_ENABLED
constexpr ProviderType kKeylessProvider = ProviderType::kSoftware;
#else
constexpr ProviderType kKeylessProvider = ProviderType::kHardware;
#endif

#if !defined(SCORE_CRYPTO_SOFTWARE_BACKEND_ENABLED) && !defined(SCORE_CRYPTO_HARDWARE_BACKEND_ENABLED)
static_assert(false, "At least one crypto backend must be enabled for the key permission tests");
#endif

/// MAC is the one keyed operation both providers implement, so the MAC cases run
/// against each one the build enables. Enforcement is provider-independent — it happens in the mediator
...
constexpr ProviderType kMacProviders[] = {
#ifdef SCORE_CRYPTO_SOFTWARE_BACKEND_ENABLED
...
// Everything up to the key slot cases below is cipher or signature work, which
// only the OpenSSL provider implements.
#ifdef SCORE_CRYPTO_SOFTWARE_BACKEND_ENABLED
...
#endif  // SCORE_CRYPTO_SOFTWARE_BACKEND_ENABLED
...
INSTANTIATE_TEST_SUITE_P(PerProvider,
                         ...
                         ::testing::Values(
#ifdef SCORE_CRYPTO_SOFTWARE_BACKEND_ENABLED
                             SlotPolicyTestData{"SoftwareProvider",
                                                ProviderType::kSoftware,
                             ...
                             SlotPolicyTestData{"HardwareProvider",
                                                ProviderType::kHardware,
                                                "HMAC_SHA256_IntegrationTestKey",
                                                "HMAC_SHA256_SignOnlySlot"}
#endif
                             ),
...
TEST_F(KeyPermissionTest, KeylessContextsAreUnaffected)
{
    ...
    hash_config.SetAlgorithm("SHA256").SetProviderType(kKeylessProvider);
