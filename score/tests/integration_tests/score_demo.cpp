...
constexpr std::size_t kHmacSha256MacSize = 32;
...
constexpr std::string_view kSlotNameSoftware = "HMAC_SHA256_IntegrationTestKey_OpenSSL";
#endif
#ifdef SCORE_CRYPTO_HARDWARE_BACKEND_ENABLED
constexpr std::string_view kSlotNameHardware = "HMAC_SHA256_IntegrationTestKey";
#endif

// Test data
constexpr std::string_view kTestDataRelativePath = "/mac/input_hello_world.bin";
constexpr std::string_view kExpectedMacRelativePath = "/mac/hmac_sha256_hello_world.bin";
...
score::Result<std::monostate> ComputeMacWithProvider(
    const std::string_view slot_name,
    ProviderType provider,
    const std::vector<uint8_t>& message,  // NOLINT(bugprone-easily-swappable-parameters)
    const std::vector<uint8_t>& expected_mac)
{
    ...
    if (!stack_result.has_value())
    {
        ...
        return score::Result<std::monostate>{score::unexpect, stack_result.error()};
    }
    ...
    if (!ctx_result.has_value())
    {
        ...
        return score::Result<std::monostate>{score::unexpect, ctx_result.error()};
    }
    ...
    auto slot_result = ctx->ResolveResource(std::string{slot_name}, ResourceType::kKeySlot);
    if (!slot_result.has_value())
    {
        print_error("ERROR: Failed to resolve slot \"" + std::string{slot_name} + "\"");
        return score::Result<std::monostate>{score::unexpect, slot_result.error()};
    }

    print_success("Resolved KeySlot resource: " + std::string{slot_name});
    ...
    print_colored("- With KeySlot: " + std::string{slot_name}, Color::BrightCyan);
    ...
    mac_config.SetAlgorithm(std::string{kAlgHmacSha256}).SetKey(slot_result.value());
    auto mac_result = ctx->CreateMacContext(mac_config);
    if (!mac_result.has_value())
    {
        print_error("ERROR: Failed to create MAC context");
        return score::Result<std::monostate>{score::unexpect, mac_result.error()};
    }
    ...
    if (!init_result.has_value())
    {
        ...
        return score::Result<std::monostate>{score::unexpect, init_result.error()};
    }
    ...
    if (!update_result.has_value())
    {
        ...
        return score::Result<std::monostate>{score::unexpect, update_result.error()};
    }
    ...
    if (!finalize_result.has_value())
    {
        ...
        return score::Result<std::monostate>{score::unexpect, finalize_result.error()};
    }
    ...
    return score::Result<std::monostate>{score::unexpect, CryptoErrorCode::kInternalError};
...
// NOLINTNEXTLINE(bugprone-exception-escape) - demo dependencies may throw; failures are handled by result checks.
int main() noexcept
{
    ...
    auto test_data = tests::utility::read_bin(GetTestVectorPath(kTestDataRelativePath));
    if (test_data.empty())
    {
        print_error("Failed to read test data from: " + GetTestVectorPath(kTestDataRelativePath));
        return 1;
    }

    auto expected_mac = tests::utility::read_bin(GetTestVectorPath(kExpectedMacRelativePath));
    if (expected_mac.empty())
    {
        print_error("Failed to read expected MAC from: " + GetTestVectorPath(kExpectedMacRelativePath));
        return 1;
    }
    ...
    auto result_openssl = ComputeMacWithProvider(kSlotNameSoftware, ProviderType::kSoftware, test_data, expected_mac);
    ...
    auto result_softhsm = ComputeMacWithProvider(kSlotNameHardware, ProviderType::kHardware, test_data, expected_mac);
